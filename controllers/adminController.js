...
module.exports = {
  ...
  verifyLogin: async (req, res) => {
    try {
      ...
      if (userData) {
        ...
        res.render("admin_login", { message: "Admin not " });
      }
    } catch (error) {
      console.log(error.message);
      res.redirect("/500");
  ...
  loadDashboard: async (req, res) => {
    try {
      ...
      const categoryNames = JSON.stringify(
        orderStats.map((stat) => stat.categoryName).flat()
      );
      const orderCounts = JSON.stringify(
        orderStats.map((stat) => stat.orderCount)
      );

      const monthlyData = Array.from({ length: 12 }, (_, index) => {
        const monthOrderData = orderData.find(
          (item) => item._id.month === index + 1
        ) || { totalOrders: 0, totalProducts: 0 };
        const monthUserData = userData.find(
      ...
      const totalOrdersJson = JSON.stringify(
        monthlyData.map((item) => item.totalOrders)
      );
      const totalProductsJson = JSON.stringify(
        monthlyData.map((item) => item.totalProducts)
      );
      const totalRegisterJson = JSON.stringify(
        monthlyData.map((item) => item.totalRegister)
      );
  ...
  getChartData: async (req, res) => {
    try {
      ...
      const categoryNames = orderStats.map(
        (stat) => stat.categoryName || "Unknown Category"
      );
  ...
  salesReport: async (req, res) => {
    try {
      ...
      const populatedOrderData = await Order.populate(orderData, {
        path: "userId",
        select: "name",
      });
    ...
    try {
      ...
      const orders = orderData.map((order) => ({
        ...
        totalDiscountPercentage: order.totalDiscountPercentage
          ? order.totalDiscountPercentage.toFixed(2) + "%"
          : "N/A",
      ...
      const templatePath = path.join(
        __dirname,
        "../../../views/admin/salesReportTemplate.ejs"
      ); // Adjusted path
