...
module.exports = {
  addCategoryOffer: async (req, res) => {
    try {
      ...
      const formattedCategoryData = category.map((cat) => {
        return {
          ...cat._doc,
          formattedExpiry: cat.expiry
            ? moment(cat.expiry).format("ddd MMM DD YYYY")
            : "",
        };
      });
  ...
  updateCategoryOffer: async (req, res) => {
    try {
      ...
      const categoryData = await Category.findByIdAndUpdate(
        { _id: categoryId },
        { $set: { discount: discount, expiry: expiry, offerStatus: true } }
      );
  ...
  deleteCategoryOffer: async (req, res) => {
    try {
      ...
      const categoryData = await Category.findByIdAndUpdate(
        { _id: categoryId },
        { $set: { discount: null, expiry: null, offerStatus: false } }
      );
  ...
  editCategoryOffer: async (req, res) => {
    try {
      ...
      const formattedExpiry = specificCategory.expiry
        ? moment(specificCategory.expiry).format("YYYY-MM-DD")
        : "";
